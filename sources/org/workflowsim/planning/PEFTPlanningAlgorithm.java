...
/**
 * The PEFT planning algorithm.
 *
 * @author Tzuhao Chu
 * @date   Jun, 2013
 */
 
 public class PEFTPlanningAlgorithm extends BasePlanningAlgorithm{
	...
	private Map<Task, Map<CondorVM, Double>> optimisticCostTable;
	...
	@Override
	public void run() {
		Log.printLine("DEFT planner running with " + getTaskList().size()
                + " tasks.");
     ...
     * º∆À„OCT±Ì
     */
	private void calculateOptimisticCostTable(){
		//º∆À„OPT
		for (Task task : getTaskList()) {
			Map<CondorVM, Double> optCostsVm = new HashMap<>();
			for (Object vmObject : getVmList()) {
				CondorVM vm = (CondorVM) vmObject;
				optCostsVm.put(vm, calculateOptimisticCostTable(task, vm));
			}
			optimisticCostTable.put(task, optCostsVm);
		}
	}
	
	private double calculateOptimisticCostTable(Task task, CondorVM vm) {
		double max = 0.0, min = Double.MAX_VALUE;
		
		for (Task child : task.getChildList()) {
			for (Object vmObject :	getVmList()) {
				CondorVM childVm = (CondorVM) vmObject;
				double childOctCost = computationCosts.get(child).get(childVm)
						+ calculateOptimisticCostTable(child, childVm);
				if (vm.getId() != childVm.getId()) {
					childOctCost += transferCosts.get(task).get(child);
				}
				min = Math.min(min, childOctCost);
			}
	...
	private void calculateOctRanks() {
        for (Task task : getTaskList()) {
            calculateOctRank(task);
        }
    }

    /**
     * Populates rank.get(task) with the rank of task as defined in the PEFT
     * paper.
     *
     * @param task The task have the rank calculates
     * @return The rank
     */
    private double calculateOctRank(Task task) {
        if (rank.containsKey(task)) {
            return rank.get(task);
        }
        
        double averageOct = 0.0;
        for (Object vmObject : getVmList()) {
        	CondorVM vm = (CondorVM) vmObject;
        	averageOct += optimisticCostTable.get(task).get(vm);
        }
        
        rank.put(task, averageOct / getVmList().size());
    ...
    private void allocateTask(Task task) {
        ...
        double finishTime;
        double optEFT = Double.MAX_VALUE;
        ...
        for (Object vmObject : getVmList()) {
            ...
            if(finishTime + optimisticCostTable.get(task).get(vm) 
            		< optEFT) {
